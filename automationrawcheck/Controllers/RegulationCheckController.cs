...
public sealed class RegulationCheckController : ControllerBase
{
    ...
    public IActionResult Health() =>
        Ok(new
        {
            ...
            note = "¹ı±Ô °ËÅä ¹é¿£µå API Á¤»ó µ¿ÀÛ Áß. °á°ú´Â Âü°í¿ë 1Â÷ ÆÇÁ¤ÀÔ´Ï´Ù."
        });

    [HttpGet("use-profiles")]
    [ProducesResponseType(typeof(List<UseProfileListItemDto>), StatusCodes.Status200OK)]
    public IActionResult GetUseProfiles()
    {
        var profiles = UseProfileRegistry.All
            .OrderBy(static profile => profile.Identity.DisplayName, StringComparer.Ordinal)
            .Select(profile => new UseProfileListItemDto
            {
                ...
                RequiredStandardInputs = profile.RequiredInputsByLevel.TryGetValue("standard", out var standard)
                    ? standard.ToList()
                    : [],
                RequiredDetailedInputs = profile.RequiredInputsByLevel.TryGetValue("detailed", out var detailed)
                    ? detailed.ToList()
                    : [],
